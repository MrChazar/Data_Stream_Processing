...
f_values, fft_mag = plot_fft(t, y, init_fs)
...
# Tworzenie suwaków
freq_slider = Slider(ax=axfreq, label='f (Częstotliwość)', valmin=1, valmax=50, valstep=1, valinit=init_f)
fs_slider = Slider(ax=axfs, label='fs (Częstotliwość próbkowania)', valmin=10, valmax=100,  valstep=1,  valinit=init_fs, orientation="vertical")
...
def update(val):
    ...
    ax2.clear()
    ax2.plot(f_values, fft_mag)
    ax2.set_xlim(0, fs/2)  # Stały zakres osi x dla widma
    ax2.set_ylim(0, max(fft_mag) * 1.1)  # Automatyczne dostosowanie granic osi y do wartości sygnału
    ax2.set_title("Widmo Sygnału")
    ax2.set_xlabel('Częstotliwość [Hz]')
    ax2.set_ylabel('Amplituda')
